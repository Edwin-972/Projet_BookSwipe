...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('DOMContentLoaded', async function () {
        ...
        if (bookId) {
            try {
                const response = await fetch(`http://localhost:3001/api/annonces_livres/${bookId}`);
                ...
                if (response.ok) {
                    const book = JSON.parse(textResponse); // Parser le texte en JSON

                    // Mettre à jour les éléments de la page avec les détails du livre
                    document.getElementById('book-image').src = book.image ? `data:image/jpeg;base64,${book.image}` : '../images/default-book.jpg';
                    document.getElementById('book-title').textContent = book.titre;
                    document.getElementById('book-author').textContent = book.auteur;
                    document.getElementById('book-category').textContent = book.categorie;
                    document.getElementById('book-condition').textContent = book.statut;
                    document.getElementById('book-location').textContent = book.ville;
                    document.getElementById('book-seller').textContent = book.vendeur;
                    document.getElementById('description-text').textContent = book.description;
                } else {
                    throw new Error('Erreur lors de la récupération des détails du livre');
                }
            } catch (error) {
                console.error('Erreur:', error);
                alert(`Impossible de charger les détails du livre. Erreur: ${error.message}`);
            }
        } else {
            alert('ID de livre manquant dans l\'URL.');
        }
    });
